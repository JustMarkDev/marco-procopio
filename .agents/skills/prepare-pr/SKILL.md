...
- Require the issue to be `In Progress` and the branch to contain its identifier.
...
4. Link the PR to Linear and move the issue from `In Progress` to `In Review` unless integration already did so.
